...
export class Fight {
	...
	readonly defender: CombatEntity;
	readonly world: World;

	readonly attacks: AttackInfo[] = [];

	constructor(char1: CombatEntity, char2: CombatEntity, world: World) {
	...
	async doKill(char: Char, target: Npc, party?: Party) {
		...
		if (target instanceof Monster) {
			...
			char.evil = +char.evil + (-target.evil) / 2 + 1 + target.getModifier('cha');
			char.addHistory('pk');
...
class AttackInfo {
	...
	constructor(attacker: Char | Monster, defender: Npc, party?: Party) {
		...
		if (this.rollHit()) this.rollMeleeDmg();
